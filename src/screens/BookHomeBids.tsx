...
const BookHomeBids = () => {
  ...
  const validateBidAmount = (
    ...
    if (bid < range.min || bid > range.max)
      return 'Please enter a bid amount within the range';
    return null;
  };
      ...
      {contestId, timeslotId: slotId, payload},
      {
        onSuccess: data => {
          if (data?.data?.success) {
            setUpTo(prev =>
              Number(prev || '') - 1 > 0 ? Number(prev || '') - 1 : prev,
            );
  ...
  return (
    ...
    backgroundColor={colors.black}>
      ...
      <AppBar
        onBackPress={() => {
          socketServices.emit('leave-winninguser'); // triggers server-side disconnect event
        }}
        textColor={colors.gold}
        back
        title={`${'\u20B9'} ${maxPrice ? maxPrice : ''} `}
        right={<RightIcon />}
      />
      <ScrollView
        refreshControl={
        ...
        <Body>
          <Box
            ...
            {currentTimeSlot ? (
              <GameTimer
                onRoomLeft={() => {
                  socketServices.emit('leave-winninguser');
                }}
                contestId={contestId}
                slotId={slotId}
          ...
          <Box mx={moderateScale(15)} gap={moderateScale(20)}>
            <InputText
              label={`Bid between ${rangeAmount?.min}${
                decimalRange.trim() ? '.' + decimalRange : decimalRange
              } - ${rangeAmount?.max}${
                decimalRange.trim() ? '.' + decimalRange : decimalRange
              }`}
              textInputProps={{
            ...
            <PrimaryButton
              ...
              loading={useJoinBidHomeMutation?.isPending || btnLoading}
              disabled={useJoinBidHomeMutation?.isPending}
