...
import { ArrowDownIcon, ArrowUpIcon, Box, ChevronDownIcon, ChevronUpIcon, Pressable, Spinner, Text, Toast, ToastTitle, useToast, View } from '@gluestack-ui/themed'
import DatePicker, { getToday, getFormatedDate } from 'react-native-modern-datepicker';
import { responsiveHeight, responsiveWidth } from 'react-native-responsive-dimensions';
import { NativeStackNavigationProp } from '@react-navigation/native-stack'
import { ParamListBase, useNavigation, useRoute } from '@react-navigation/native'
import { format, parse, parseISO, subDays, isBefore, isAfter, formatDate } from 'date-fns';
...
import { useNotifyContestInMyMatches, useSaveContestInMyMatches } from '../hooks/home/save-contest-in-mymatch';
...
const HomeContestList = () => {
  ...
  const [contestTypeFilter, setContestTypeFilter] = useState<string | undefined>("")
  ...
  const callJoinCategoryEvent = async () => {
    socketServices.emit('get-my-contest', {
  ...
  }, [enterFilter, spotsFilter, prizePoolFilter, winnerFilter, contestTypeFilter])
  ...
  useEffect(() => {
    ...
    if (cardFrom === 'live') {
      ...
      socketServices.on('mycontest-live-contest', (data: any) => {
        console.log("mycontest-live-contest", data)
      ...
      return () => {
        socketServices.removeListener('mycontest-live-contest')
      }
    }

    if (cardFrom === 'wining') {
      console.log("helo")
      socketServices.on('mycontest-expired-contest', (data: any) => {
        console.log("mycontest-upcoming-category", data)
      ...
      return () => {
        socketServices.removeListener('mycontest-expired-contest')
      }
    }
    ...
    if (cardFrom === 'upcoming') {
      ...
      socketServices.on('mycontest-upcoming-contest', (data: any) => {
        setContests((prevContests: any) => ([
          ...data,
        ]));
      })
      return () => {
        socketServices.removeListener('mycontest-upcoming-contest')
      }
    }
  ...
  const handleSaveContest = (contestId: string, categoryId: string, slotId: string) => {
    useSaveContestInMyMatchesMutation.mutate({ contestId: contestId, categoryId, timeslotId: slotId }, {
      onSuccess: (data) => {
  ...
  const handleNotifyContest = (contestId: string, categoryId: string, slotId: string) => {
    useNotifyContestInMyMatchesMutation.mutate({ contestId: contestId, categoryId, timeslotId: slotId }, {
      onSuccess: (data) => {
        if (data?.data?.success) {
          toast.show({
            placement: "bottom",
  ...
  const LeftAppBar = () => {
    return (
      <Box flexDirection='row' alignItems='center' px={moderateScale(10)} gap={moderateScale(15)}>
        ...
        <Box>
          <Text fontFamily={'$robotoBold'} fontSize={16} lineHeight={18} color={colors.white} numberOfLines={1}>{categoryName ?? 'N/A'}</Text>
        </Box>
      </Box>
    )
  }
  ...
  const newArrayLength = contests.reduce((acc, crr) => {
    ...
    console.log("geting length.......", crr.contests.length)
    return acc;
  }, 0);

  // console.log('contests[0]?.isNotificationActive', contests[0]?.contests[0]?.isNotificationActive)
  // console.log('contests[0]?.isUserBookMarked', contests[0]?.contests[0]?.isUserBookMarked)

  console.log(contests)

  return (
    <LinearGradient colors={[colors.black, colors.gold2]} locations={[0.65, 1]} useAngle angle={205} angleCenter={{ x: -0.2, y: 0.1 }} style={{ flex: 1 }}>
      ...
      <Container statusBarStyle='light-content' statusBarBackgroundColor={colors.themeRed} backgroundColor='black' >
        ...
        {/* <Box display={cardFrom === 'upcoming' || cardFrom === 'wining' ? 'flex' : 'none'} flexDirection='row' alignItems='center' justifyContent='space-between' pl={moderateScale(15)} pr={moderateScale(10)} bgColor={colors.black} borderBottomWidth={2} borderBottomColor={colors.gray6}>
          ...
          <Box flexDirection='row' alignItems='center' gap={1}>
            <Pressable flexDirection='row' alignItems='center' gap={3} onPress={() => { setDatePickerModel(!datePickerModel) }}>
              <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.grayish} numberOfLines={1} py={10}>{startDateFilter === '' ? 'Select Date' : `${startDateFilter}`}</Text>
              {datePickerModel ? <Icon as={ChevronUpIcon} w="$4" h="$4" color={colors.white} /> : <Icon as={ChevronDownIcon} w="$4" h="$4" color={colors.grayish} />}
            </Pressable>

            <Pressable flexDirection='row' alignItems='center' gap={3} onPress={() => { setDatePickerModelEnd(!datePickerModelEnd) }}>
              <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.grayish} numberOfLines={1} py={10}>{endDate === '' ? 'Select Time' : `${endDate}`}</Text>
        ...
        <Box w={'100%'} backgroundColor='black' flexDirection='row' alignItems='center' alignContent='center' borderBottomWidth={1} borderBottomColor={colors.gray5} >
          ...
          <Pressable
            onPress={() => {
              ...
              prizePoolFilter == 'max' ? setPrizePoolFilter("min") : setPrizePoolFilter("max")
            }}
            flex={0.9}
            alignItems="center"
            flexDirection="row"
          ...
          >
            <Text
              ...
              borderBottomColor={selectedSortBy === 'prizepool' ? "gold" : "transparent"} // Active border gold kiya
              numberOfLines={1}
              py={10}
            >
              PRIZE POOL
        ...
        <Box flexDirection='row' alignItems='center' justifyContent='space-between' bgColor={colors.black} px={moderateScale(15)} py={moderateScaleVertical(10)}>
          ...
          <Box flexDirection='row' alignItems='center' gap={moderateScale(15)}>
            ...
            <TouchableOpacity onPress={handleClearFilter} ><Text fontFamily={'$robotoBold'} fontSize={16} lineHeight={18} color={colors.gray6} numberOfLines={1}>CLEAR</Text></TouchableOpacity>
          </Box>
        </Box>
        ...
        <Box flex={1}>
          <FlatList
            ...
            renderItem={({ item, index }: { item: MAIN_CATEGORY, index: number }) => {
                ...
                <Box key={item?._id} backgroundColor={colors.black} py={moderateScaleVertical(10)}>
                  ...
                  <Box gap={15} backgroundColor='black'>
                    {item?.contests?.length > 0 ? (<>
                      {/* <Text style={{color:colors.gold,textAlign:"center" ,marginTop:200}}>{item?.contests?.length}</Text> */}
                      {
                        item?.contests?.map((item: CONTEST, index: number) => {
                          ...
                          return (
                            <Box backgroundColor='black' key={index?.toString()} px={moderateScale(15)}>
                                ...
                                <Pressable gap={8} onPress={() => navigation.navigate(NavigationString.ViewHomeContest, { flexible: true ? 'Yes' : 'No', cardFrom: cardFrom, contestId: item?._id, slotId: item?.timeSlots?._id, isUserJoinContest: item?.isUserJoinContest })} >
                                  <Box display={cardFrom === 'upcoming' ? 'flex' : 'none'} flexDirection='row' alignItems='center' justifyContent="space-between" px={moderateScale(10)}>
                                    <Pressable style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", }} hitSlop={20} onPress={() => { handleSaveContest(item?._id, cotegoryId, item?.timeSlots?._id) }}>
                                      {item?.isUserBookMarked ? <Icon as={FavouriteIcon} fill={colors.white} w="$5" h="$5" color={colors.white} /> : <Icon fill={colors.black} as={FavouriteIcon} w="$5" h="$5" color={colors.white} />}
                                      <Text style={{ color: colors.white, fontSize: 14, marginLeft: 5, marginBottom: 2 }}>{item?.favorite}</Text>
                                    </Pressable>
                                    {/* <Pressable hitSlop={20} onPress={() => { handleSaveContest(item?._id, cotegoryId, item?.timeSlots?._id) }}>
                                      {item?.isNotificationActive ? <Icon as={BellIcon} fill={colors.white} w="$5" h="$5" color={colors.white} /> : <Icon as={BellIcon} w="$5" h="$5" color={colors.white} />}
                                    </Pressable> */}
                                  </Box>
                                    ...
                                    <Box alignItems='center' flex={1} >
                                      ...
                                      {cardFrom === 'upcoming' && <>
                                        <Text fontFamily="$robotoMedium" fontSize={12} lineHeight={14} color={colors.dimGray} numberOfLines={2} textAlign="center">
                                          {format(item?.timeSlots?.startTime, "yyyy-MM-dd") === format(new Date(), "yyyy-MM-dd") ? <CardTimer endTime={item?.timeSlots?.startTime} color={colors.red} /> : format(item?.timeSlots?.startTime, "hh:mm a")}
                                        </Text>
                                  ...
                                  <Slider disabled={true} minimumValue={0} maximumValue={item?.slots} maximumTrackTintColor={'#fdebeb'} minimumTrackTintColor={colors.gold} maximumTrackStyle={{ height: moderateScale(6) }}
                                    minimumTrackStyle={{ height: moderateScale(6) }} thumbTintColor="transparent" value={item?.slotsContestFillInfo} onValueChange={(value) => { }}
                                  ...
                                  <Box flexDirection="row" alignItems="center" justifyContent='space-between' px={10}>
                                    <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.white} numberOfLines={1}>
                                      {(() => {
                                        const totalSlots = item?.slots ?? 0; // Default to 0 if undefined
                                        const filledSlots = item?.slotsContestFillInfo ?? 0; // Default to 0 if undefined
                                        const leftSlots = totalSlots - filledSlots; // Calculate left slots
                                        return `${leftSlots > 0 ? leftSlots : 0} spots left`; // Ensure no negative values
                                      })()}</Text>

                                    <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.gray4} numberOfLines={1} > {item?.slots == 'undefined' ? 0 : item?.slots}  spots</Text>
                                  </Box>
                                </Pressable>
                                <Box flexDirection="row" alignItems="center" justifyContent='space-between' backgroundColor={colors.black} px={10} py={15}>
                                  <Box flexDirection="row" alignItems="center" gap={10} backgroundColor='black'>
                                    <Tooltip actionType='press' withOverlay={false} backgroundColor={colors.themeBlue} height={moderateScale(30)} popover={<Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.white} numberOfLines={1} >First Prize = {'\u20B9'}{formatAmount(item?.prizeDistribution[0]?.prizeAmount)}</Text>}>
                                      <Box flexDirection="row" alignItems="center" gap={3}>
                                        <Image alt="icon" source={imgIcon.prize1} w={moderateScale(15)} h={moderateScale(15)} resizeMode="contain" />
                                        <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.gray4} numberOfLines={1} >{'\u20B9'}{formatAmount(item?.rankDistribution[0]?.amount)}</Text>
                                      </Box>
                                    </Tooltip>
                                    <Tooltip actionType='press' withOverlay={false} backgroundColor={colors.themeBlue} height={moderateScale(30)} width={moderateScale(170)} popover={<Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.white} numberOfLines={1} >{item?.rankPercentage}% Bids win the contest</Text>}>
                                      <Box flexDirection="row" alignItems="center" gap={3}>
                                        <MyMatch15Icon />
                                        <Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.gray4} numberOfLines={1} >{item?.rankPercentage}%</Text>
                                      </Box>
                                    </Tooltip>
                                    <Tooltip actionType='press' withOverlay={false} backgroundColor={colors.themeBlue} height={moderateScale(50)} width={moderateScale(170)} popover={<Text fontFamily={'$robotoMedium'} fontSize={12} lineHeight={14} color={colors.white} numberOfLines={2} >Max {item?.upto} entries per user in this contest</Text>}>
                                      <Box flexDirection="row" alignItems="center" gap={3}>
        ...
        <Modal animationType='slide' transparent={true} visible={unfavorite} onRequestClose={() => setUnfavorite(false)}>
          <Box flex={1} backgroundColor='rgba(0, 0, 0, 0.5)' alignItems='center' justifyContent='center'>
            <Box backgroundColor={colors.white} borderRadius={10} w={'90%'} gap={15} py={20}  >

              {/* Are you sure you want to turn off notifications for these Contest */}
              <Text fontFamily={'$robotoMedium'} fontSize={16} lineHeight={18} color={colors.black} numberOfLines={3} mx={moderateScale(10)} textAlign='center'>Are you sure ,you want to remove this contest from your favorites?</Text>


              <Box flexDirection='row' alignItems='center' mx={moderateScale(10)} gap={moderateScale(15)}>
                <PrimaryButton buttonText='Yes' onPress={() => { setUnfavorite(false) }} backgroundColor={colors.gray7} height={moderateScale(45)} flex={1} alignSelf='center' marginTop={moderateScaleVertical(5)} />
                <PrimaryButton buttonText='No' onPress={() => { }} backgroundColor={colors.themeRed} height={moderateScale(45)} flex={1} alignSelf='center' marginTop={moderateScaleVertical(5)} />
              </Box>
        ...
        <Modal animationType='slide' transparent={true} visible={showFilterModel} onRequestClose={() => setShowFilterModel(false)}>
          <Box flex={1} backgroundColor='rgba(9, 1, 1, 0.5)' >
            <Box backgroundColor={colors.black} borderTopLeftRadius={10} borderTopRightRadius={10} w={'100%'} gap={15} h={moderateScale(640)} marginTop={deviceHeight * 0.4} overflow='hidden'>
              <Box flexDirection='row' justifyContent='space-between' alignItems='center' bgColor={colors.black} borderWidth={0.3} borderColor='white' py={moderateScaleVertical(15)} px={moderateScale(15)}>
                <Pressable onPress={() => setShowFilterModel(false)}>
                  <Icon as={CloseIcon} size="lg" color={colors.red} />
                </Pressable>
                <Text fontFamily='$robotoBold' fontSize={16} lineHeight={18} color={colors.white} numberOfLines={1} >Filter</Text>
                <TouchableOpacity onPress={handleClearFiltterRange}>
                  <Text fontFamily='$robotoBold' fontSize={16} lineHeight={18} color={colors.white} numberOfLines={1} >CLEAR</Text>
                </TouchableOpacity>
              </Box>
              ...
              <Body>
                <Box px={moderateScale(15)} borderBottomColor={colors.gray5} py={moderateScaleVertical(15)}>
                  ...
                  <Box flexDirection='row' gap={moderateScale(10)} flexWrap='wrap' mt={moderateScaleVertical(10)}>
                    <TouchableOpacity style={{ borderRightWidth: 1, borderLeftWidth: 1, borderTopLeftRadius: 10, borderBottomLeftRadius: 10, borderTopRightRadius: 10, borderBottomRightRadius: 10, borderColor: colors.white, paddingHorizontal: 5, paddingVertical: 6 }} onPress={() => handleSlotFilter(1, 1000, "slot")} >
                      <Text fontFamily='$robotoMedium' fontSize={14} lineHeight={16} color={colors.white} numberOfLines={1} >0 - 1000</Text>
                    </TouchableOpacity>

                    <TouchableOpacity style={{ borderRightWidth: 1, borderLeftWidth: 1, borderTopLeftRadius: 10, borderBottomLeftRadius: 10, borderTopRightRadius: 10, borderBottomRightRadius: 10, borderColor: colors.white, paddingHorizontal: 5, paddingVertical: 6 }} onPress={() => handleSlotFilter(10, 10000, "slot")} >
