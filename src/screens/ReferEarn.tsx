...
const ReferEarn = () => {
 ...
 const  { data: referalData, isLoading: profileIsLoading }=  useUserReferal()
  ...
  <Pressable
    onPress={() => handleCopyToClipboard(referalData?.data?.referalCode)}
    style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]} // Press feedback
  >
    <Box
      flexDirection="row"
      alignItems="center"
      ...
      borderRadius={8}
    >
      <Text fontSize={18} color={colors.grayish} textAlign="center">
        {referalData?.data?.referalCode}
      </Text>
